import { FiCamera, FiCheckCircle, FiClock, FiFlag, FiMapPin, FiStar, FiTrash2 } from "react-icons/fi";
...
export default function PropertyCard({ p, onView, user, onDel, isFavorite = false, onToggleFavorite }) {
  ...
  return (
    <div className={`pcard${p.boosted ? " boosted" : ""}${p.status === "taken" ? " taken" : ""}${p.flagged ? " flagged" : ""}`}>
      {p.boosted && <div className="crown"><FiStar aria-hidden="true" /> Featured Listing</div>}
      <button type="button" className="pthumb" onClick={() => onView(p)} aria-label={`View details for ${p.title}`}>
        ...
        {p.flagged && <div className="pflag"><FiFlag aria-hidden="true" /> Flagged</div>}
        {p.media?.length > 0 && <div className="pmcnt"><FiCamera aria-hidden="true" /> {p.media.length}</div>}
      </button>
      <div className="pbody">
        <div className="ptitle">{p.title}</div>
        <div className="ploc"><FiMapPin aria-hidden="true" /> {p.location}</div>
        <div className="ptrust"><span className="pverified">{p.approved ? <><FiCheckCircle aria-hidden="true" /> Verified listing</> : <><FiClock aria-hidden="true" /> Pending approval</>}</span><span className={`pavailability ${p.status === "available" ? "available" : "taken"}`}>{p.status === "available" ? "Available" : p.status === "pending" ? "Pending" : "Taken"}</span></div>
        <div className="pmeta">
        ...
        <div className="pfoot">
          ...
          {canDel && <button className="pdbtn" title="Delete property" onClick={e => { e.stopPropagation(); onDel(p.id); }} aria-label={`Delete ${p.title}`}><FiTrash2 aria-hidden="true" /></button>}
        </div>
      </div>
    </div>
  );
